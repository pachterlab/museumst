...
c(inst_gc, city_gc) %<-% geocode_inst_city(publications, cache_location = "inst")
...
```{r}
species_img <- data("species_img")
```

```{r}
species <- sheet %>% 
  select(year, title, species, country, city, institution) %>% 
...
```{r}
langs <- sheet %>% 
  select(year, date_published, title, journal, language, documented:`CRAN/Bioc/pip/conda`) %>% 
...
langs %>% 
  ...
  pubs_per_cat(language,
               isotype = FALSE) #isotype = TRUE, img_unit = 2)
```
