...
events %>% 
  ...
  plot_timeline(c(0.6, -0.8, 0.6, -0.8, 0.4, -1.2, 0.5, -0.5, 1, -1, 0.8, -0.5, 1.1, -0.8, 0.5), 
                expand_x = c(0.1, 0.1), expand_y = c(0.05, 0.05))
```
...
species %>% 
  ...
  pubs_per_cat(category = "species", isotype = TRUE, img_unit = 10)
```
...
```{r}
plot_wordcloud(sheet, col_use = "tissue", species_use = "Drosophila melanogaster")
```

```{r}
plot_wordcloud(sheet, col_use = "tissue", species_use = "Mus musculus")
```
...
```{r}
hist_bool_line(sheet, still_available)
```
...
Sort of suggestive that more recent atlases are more likely to be still available. 
