...
```{r}
no_preprints <- publications %>% 
  filter(!journal %in% c("bioRxiv", "arXiv"))
ggplot(no_preprints, aes(date_published, color = is_prequel)) +
...
```{r}
days_from_start <- publications %>% 
  group_by(is_prequel) %>% 
  mutate(days_since_first = as.numeric(date_published - min(date_published)))
ggplot(days_from_start, aes(days_since_first, color = is_prequel)) +
  geom_freqpoly(binwidth = 365)
```
...
```{r}
no_preprints <- analysis_sheets %>% 
  filter(!journal %in% c("bioRxiv", "arXiv"))
ggplot(no_preprints, aes(date_published, color = sheet)) +
  geom_freqpoly(binwidth = 365)
```

```{r}
days_from_start <- analysis_sheets %>% 
  group_by(sheet) %>% 
  mutate(days_since_first = as.numeric(date_published - min(date_published)))
ggplot(days_from_start, aes(days_since_first, color = sheet)) +
  geom_freqpoly(binwidth = 365)
```
