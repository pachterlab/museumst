...
usethis::use_data(lcm_city_gc)
lcm_abstracts <- abstracts
usethis::use_data(lcm_abstracts)
```
...
As expected, fewer topics results into higher semantic coherence and lower exclusivity. This is really a trade off. Sort of a compromise, here I choose the model with K = 40. Since I have held out part of the data when choosing K, I'll use this K to fit the model with the full data.

```{r}
stm_res <- stm(ac_dfm2, K = 40, prevalence = ~ date_num + city_eff,
...
```{r}
plot(stm_res, type = "perspectives", topics = c(4, 40))
```

```{r}
lcm_dfm2 <- ac_dfm2
usethis::use_data(lcm_dfm2)
...
Remember the histogram over time. There are two peaks. The topic proportions here are from the fitted model, and what's plotted is the spline smoothed topic proportion over time. Because of the smoothing, there can be some negative values. That's why I prefer histograms to density plots that use kernel smoothing. 
...
```{r}
year_means %>% 
  filter(topic %in% as.character(c(1,2,5,6,16,21:23,37))) %>% 
...
```{r}
labelTopics(stm_res, 22)
```

```{r}
findThoughts(stm_res, topics = 22, texts = abstracts$abstract, n = 5)
```

Topic 1: cancer marker genes from transcriptomics
Topic 5: prostate cancer
Topic 21: microarray technicalities
Topic 6: cancer cell genome stability
Topic 23: cancer proteomics
Topic 2: neurological disorders
Topic 16: miRNAs, especially in the kidney
Topic 38: development of flowers and leaves
Topic 14: bone growth plate
Topic 31: infectious diseases, especially in plants
Topic 37: plant seed development
Topic 22: plant roots
